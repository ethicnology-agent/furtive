...
class ActivitiesListPage extends StatefulWidget {
  const ActivitiesListPage({super.key, this.activities, this.pickGpx});

  final ActivityRepository? activities;
  final Future<XFile?> Function()? pickGpx;
...
class _ActivitiesListPageState extends State<ActivitiesListPage> {
  late final _activities = widget.activities ?? ActivityRepository();
  ...
  Future<void> _pickAndImportGpx() async {
    if (_pickingImport ||
        context.read<ActivitiesBloc>().state.importStatus ==
            ActivityImportStatus.inProgress) {
      return;
    }
    setState(() => _pickingImport = true);
    ...
    try {
      ...
      if (mounted) {
        messenger.showSnackBar(
          SnackBar(content: Text(l10n.importPickerFailed)),
        );
      }
    } finally {
      if (mounted && !queued) setState(() => _pickingImport = false);
    }
  }
    ...
    return BlocListener<ActivitiesBloc, ActivitiesState>(
      ...
      listener: (context, state) {
        if (state.importStatus == ActivityImportStatus.success ||
            state.importStatus == ActivityImportStatus.failure) {
          setState(() => _pickingImport = false);
        }
        if (state.error != null) {
          ...
          actions: [
            BlocBuilder<ActivitiesBloc, ActivitiesState>(
              builder: (context, state) => IconButton(
                ...
                onPressed:
                    _pickingImport ||
                        state.importStatus == ActivityImportStatus.inProgress
                    ? null
                    : _pickAndImportGpx,
              ),
            ),
